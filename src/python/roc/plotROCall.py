...
inFileLSH = sys.argv[1]
inFileSNR = sys.argv[2]
outFile = sys.argv[3]

dfLSH = pd.read_csv(inFileLSH)
print(dfLSH)
...
plt.scatter(fprVals,tprVals,label="LSH",color="C0")
plt.xlim([0,1])
plt.ylim([0,1])
plt.plot(np.linspace(0,1),np.linspace(0,1),'k--',alpha=0.5)
...
dfSNR = pd.read_csv(inFileSNR)

plt.plot(dfSNR.tpr.values,dfSNR.fpr.values,label="SNR",color="C1")
